...
def _render_s3_resume_manifest(
    ...
    script_lines = [
        "set -e",
        "",
        "# Locate PRAGMA scripts dir.",
        "# Wheel install: pragma_encoder is in site-packages; scripts/ is at WORKDIR.",
        "# WORKDIR=/opt/app-root/src in the S2I base image.",
        "PRAGMA_ROOT=''",
        "for CANDIDATE in /opt/app-root/src /opt/app-root/src/pragma-encoder /pragma-encoder .; do",
        "  if [ -f $CANDIDATE/scripts/train_pragma.py ]; then",
        "    PRAGMA_ROOT=$CANDIDATE",
        "    break",
        "  fi",
        "done",
        "if [ -z $PRAGMA_ROOT ]; then",
        "  echo 'ERROR: Cannot find scripts/train_pragma.py' >&2; exit 1",
        "fi",
        ...
        "",
        "# Fit tokenizer — invoke as a wheel module (no src/ tree in wheel image).",
